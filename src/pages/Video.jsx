...
const Video = () => {
  ...
  const { data: playlists, isLoading: isLoadingPlaylists } = useQuery({
    queryKey: ["playlists"],
    queryFn: async () =>
      (await axiosInstance.get(`/playlist/playlists/${user?._id}`)).data,
    staleTime: 1000 * 60 * 5,
  ...
  const addToPlaylistMutation = useMutation({
    mutationFn: async ({ playlistId, videoId }) =>
      (await axiosInstance.post(`/playlist/addvideo/${playlistId}/${videoId}`))
        .data,
    onSuccess: (data) => {
    ...
    onError: (error) =>
      toast.error(error?.response?.data || "Error adding video to playlist"),
  });
  ...
  const handleAddToPlaylist = () =>
    selectedPlaylist &&
    addToPlaylistMutation.mutate({ playlistId: selectedPlaylist, videoId });
  ...
  if (isError)
    return <p className="text-red-500">Error loading video try again</p>;

  const { likes, title, views, channelId, ownerAvatar, owner, _id, createdAt } =
    video;
    ...
    <main className="video-page flex flex-col items-center justify-center py-8 px-4 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 min-h-screen text-gray-200">
      <section className="w-full sm:w-11/12 md:w-10/12 lg:w-9/12 xl:w-8/12 rounded-2xl overflow-hidden shadow-2xl mb-8 transform transition duration-300 hover:scale-105 hover:shadow-emerald-500/50">
        <video
          controls
          src={video.video}
          className="w-full h-auto aspect-video rounded-xl bg-black"
        />
      </section>
      ...
      <section className="bg-slate-800 w-full sm:w-11/12 md:w-10/12 lg:w-9/12 xl:w-8/12 p-6 rounded-2xl shadow-lg transition duration-300 hover:shadow-xl hover:bg-slate-700">
          ...
          <div className="space-y-2">
            ...
            <p className="flex items-center gap-3">
              ...
              <span
                ...
                className="px-4 py-2 text-sm font-semibold text-white  rounded-full shadow hover:bg-blue-800 hover:shadow-lg transition duration-200  bg-slate-800"
              >
                {" "}
                <ThumbsUp />{" "}
              </span>{" "}
            ...
            <Link
              ...
              className="flex items-center gap-3 mt-3 sm:mt-4 hover:underline"
            >
              <img
                src={ownerAvatar}
                alt="owner avatar"
        ...
        <section className="bg-slate-700 p-4 rounded-lg mt-6 shadow-inner transition duration-300 hover:bg-slate-600">
          ...
          <label
            htmlFor="toggleDesc"
            className="block text-lg font-semibold cursor-pointer text-white peer-checked:text-gray-400 hover:text-emerald-400"
          >
            Show Description
          </label>
          <p className="text-sm text-gray-300 mt-3 hidden peer-checked:block">
            {video.description}
          </p>
        </section>
      </section>
