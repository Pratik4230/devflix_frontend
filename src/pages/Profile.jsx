...
const Profile = () => {
  ...
  const handleUpdateImage = (imageFile, mutation) => {
    ...
    if (imageFile) {
      formData.append(imageFile === avatarImage ? 'avatar' : 'coverImage', imageFile);
  ...
  return (
    ...
    {isEditingAvatar && (
      <section className="mt-5 bg-white dark:bg-black p-5 rounded-lg shadow-md">
        ...
        />
        <button
          onClick={() => handleUpdateImage(avatarImage, updateAvatarImageMutation)}
          className="bg-blue-500 text-white px-4 py-2 rounded mt-3"
        >
          {updateAvatarImageMutation?.isPending ? <LoaderPinwheel className="animate-spin text-white" /> : "Update Avatar"}
        </button>
      </section>
    )}
    ...
    {isEditingCover && (
      <section className="mt-5 bg-white dark:bg-black p-5 rounded-lg shadow-md">
        ...
        />
        <button
          onClick={() => handleUpdateImage(coverImage, updateCoverImageMutation)}
          className="bg-blue-500 text-white px-4 py-2 rounded mt-3"
        >
          {updateCoverImageMutation?.isPending ? <LoaderPinwheel className="animate-spin text-white" /> : "Update Cover"}
        </button>
      </section>
    )}
  </main>
