...
const Postcard = ({ post }) => {
  ...
  return (
    <div className="bg-white dark:bg-black  shadow-lg rounded-lg p-6 max-w-md mx-auto transition-transform transform hover:scale-105 hover:shadow-2xl">
      ...
      <p className="text-gray-700 dark:text-gray-100 mb-5 leading-relaxed">
        {content}
      </p>

      <div className="flex justify-between items-center border-t pt-4 border-gray-100">
        <p className="text-sm text-gray-500 dark:text-fuchsia-50 font-medium">
          ❤️ {likes} Likes
        </p>
        <button
          onClick={() => handleLike(post?._id)}
          className={`px-4 py-2 text-sm font-semibold text-white  rounded-full shadow hover:bg-blue-800 hover:shadow-lg transition duration-200 ${
            isLiked ? `bg-blue-600` : `bg-slate-800`
          }`}
        >
          <ThumbsUp />
        </button>
